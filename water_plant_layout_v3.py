...
class AdvancedGA:
    ...
    def create_chromosome(self) -> Dict:
        ...
        return {'order': order, 'rotations': rotations, 'zone_order': zone_order, 'strategy': strategy}
    ...
    def _find_best_compact_pos(self, placed: List[Facility], new_f: Facility,
        ...
        # 限制搜索范围提高效率
        search_limit = min(20, len(placed))
        recent_placed = placed[-search_limit:]
        
        for pf in recent_placed:
            pw, ph = pf.get_dimensions()
        ...
        # 去重并过滤
        candidates = list(set(c for c in candidates if c[0] >= 0 and c[1] >= 0))
        ...
        best_score = float('inf')
        for x, y in candidates:
            if self._check_conflict_at(placed, new_f, x, y):
                continue
            new_max_x = max(cur_max_x, x + w)
            new_max_y = max(cur_max_y, y + h)
            score = new_max_x * new_max_y
            aspect = max(new_max_x, new_max_y) / max(min(new_max_x, new_max_y), 1)
            if aspect > 2.0:
                score += (aspect - 2.0) * 300
            if score < best_score:
                best_score = score
                best_pos = (x, y)
        if best_pos:
    ...
    def _calc_pipeline_length(self, placed: List[Facility]) -> float:
        ...
        for f in placed:
            w, h = f.get_dimensions()
            centers[f.name] = (f.x + w/2, f.y + h/2)
        
        # 管线类型权重（主工艺管线权重高）
        system_weights = {
            "main_water": 3.0,   # 主水流管线最重要
            "backwash": 1.5,     # 反冲洗管线
            "chemical": 2.0,     # 加药管线较重要
            "power": 1.0,        # 电力线路
        }
        
        for system_name, system_data in PIPELINE_SYSTEM.items():
            weight = system_weights.get(system_name, 1.0)
            for n1, n2 in system_data["pipes"]:
                if n1 in centers and n2 in centers:
                    cx1, cy1 = centers[n1]
                    cx2, cy2 = centers[n2]
                    # 曼哈顿距离（正交路径长度）* 权重
                    total_length += (abs(cx2 - cx1) + abs(cy2 - cy1)) * weight
    ...
    # ===== 公共辅助：计算两设施中心间的曼哈顿距离 =====
    @staticmethod
    def _segment_dist(fdict: dict, n1: str, n2: str) -> float:
        """计算两设施中心曼哈顿距离（管线水平投影长度估算）"""
        if n1 not in fdict or n2 not in fdict:
            return 0.0
        f1, f2 = fdict[n1], fdict[n2]
        w1, h1 = f1.get_dimensions()
        w2, h2 = f2.get_dimensions()
        return abs((f2.x + w2/2) - (f1.x + w1/2)) + abs((f2.y + h2/2) - (f1.y + h1/2))
    ...
    def _calc_elevation_constraint(self, placed: List[Facility]) -> float:
        ...
        for (n_up, n_down) in all_paths:
            ...
            # --- 管线水平距离 ---
            L = self._segment_dist(fdict, n_up, n_down)
...
def visualize(facilities: List[Facility], bw: float, bh: float, 
    ...
    # ========== 重新计算实际边界 ==========
    if facilities:
        actual_w = max(f.x + f.get_dimensions()[0] for f in facilities)
        actual_h = max(f.y + f.get_dimensions()[1] for f in facilities)
        bw, bh = actual_w, actual_h
    ...
    zone_names_en = {
        "取水区": "Water Intake",
        "预处理区": "Pretreatment",
        "常规处理区": "Conventional Treatment",
        "深度处理区": "Advanced Treatment",
        "消毒加药区": "Disinfection & Dosing",
        "送配水区": "Distribution",
        "动力区": "Power Supply",
        "污泥处理区": "Sludge Treatment",
        "行政办公区": "Administration",
        "辅助设施区": "Auxiliary Facilities",
        "生活区": "Living Area"
    }
    ...
    # 厂区背景
    ax.add_patch(Rectangle((0, 0), bw, bh, lw=2.0, ec='#2C3E50', fc='#FAFCFA', zorder=0))

    # 绘制网格
    grid_spacing = 20
    for x in range(0, int(bw) + 1, grid_spacing):
        ax.plot([x, x], [0, bh], color='#E0E0E0', lw=0.5, alpha=0.5, zorder=0.5)
    for y in range(0, int(bh) + 1, grid_spacing):
        ax.plot([0, bw], [y, y], color='#E0E0E0', lw=0.5, alpha=0.5, zorder=0.5)

    # 绘制设施
    for f in facilities:
        w, h = f.get_dimensions()
        color = CATEGORY_COLORS.get(f.category, '#CCCCCC')
        # 阴影
        shadow = Rectangle((f.x + 0.6, f.y - 0.6), w, h, fc='#00000015', ec='none', zorder=1.9)
        ax.add_patch(shadow)
        # 主体
        rect = FancyBboxPatch((f.x, f.y), w, h, boxstyle="round,pad=0.02,rounding_size=0.5", lw=1.0, ec='#333333', fc=color, alpha=0.85, zorder=2)
        ax.add_patch(rect)
        # 标签
        cx, cy = f.x + w/2, f.y + h/2
        fs = max(7, min(10, int(np.sqrt(f.area) / 2.0)))
        ax.text(cx, cy, f.name, ha='center', va='center', fontsize=fs, fontweight='bold', color='#000000', zorder=10, family='monospace')

    # 绘制工艺管线
    name2center = {f.name: (f.x + f.get_dimensions()[0]/2, f.y + f.get_dimensions()[1]/2) for f in facilities}
    pipeline_legend = []
    for key, pipeinfo in PIPELINE_SYSTEM.items():
        color = pipeinfo['color']
        width = pipeinfo['width'] * 2.2
        style = pipeinfo['style']
        label = pipeinfo['label']
        for (start, end) in pipeinfo['pipes']:
            if start in name2center and end in name2center:
                x0, y0 = name2center[start]
                x1, y1 = name2center[end]
                ax.plot([x0, x1], [y0, y1], color=color, lw=width, alpha=0.95, linestyle=style, zorder=5)
        pipeline_legend.append(Line2D([0], [0], color=color, lw=2.2, linestyle=style, label=label))

    # 创建分区图例
    legend_elements = []
    for cat in CATEGORY_NAMES:
        legend_elements.append(mpatches.Patch(facecolor=CATEGORY_COLORS.get(cat, '#CCCCCC'), edgecolor='#333333', lw=0.5, label=zone_names_en.get(CATEGORY_NAMES[cat], CATEGORY_NAMES[cat])))

    legend1 = ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1.02, 1.0), fontsize=9, framealpha=0.95, edgecolor='#666666', title='Functional Zones', title_fontsize=10)
    ax.add_artist(legend1)
    ax.legend(handles=pipeline_legend, loc='upper left', bbox_to_anchor=(1.02, 0.7), fontsize=9, framealpha=0.95, edgecolor='#666666', title='Pipelines', title_fontsize=10)

    ax.set_xlim(-5, bw + 5)
    ax.set_ylim(-5, bh + 5)
    ax.set_xlabel('X Coordinate (m)', fontsize=11, family='serif')
    ax.set_ylabel('Y Coordinate (m)', fontsize=11, family='serif')
    ax.set_title(f'Optimized Facility Layout Plan\n({bw:.0f}m × {bh:.0f}m, Utilization: {sum(f.area for f in facilities)/(bw*bh)*100:.1f}%)', fontsize=13, fontweight='bold', family='serif', pad=15)
    ax.set_aspect('equal')
    ax.grid(True, ls=':', alpha=0.3, lw=0.5)

    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white', pad_inches=0.2)
    print(f"二维布局图已保存: {save_path}")
    plt.show()
...
def comprehensive_academic_analysis(facilities: List[Facility], bw: float, bh: float, 
    ...
    layout_area = bw * bh
    utilization = total_facility_area / layout_area * 100
    
    # 分区统计
    cat_areas = {}
    cat_counts = {}
    for f in facilities:
        cat_areas[f.category] = cat_areas.get(f.category, 0) + f.area
    ...
    for n1, n2, weight in ADJACENCY_REQUIRED:
        if n1 in fdict and n2 in fdict:
            ...
            adj_distances.append((n1, n2, dist, weight))
            if dist < 5:
                adj_satisfied += 1
    
    adj_rate = adj_satisfied / len(ADJACENCY_REQUIRED) * 100 if ADJACENCY_REQUIRED else 0
    ...
    print("\n" + "=" * 78)
    print("         📊 生成全面学术论文风格可视化分析...")
    print("=" * 78)
    
    # ========== 图1: 主布局图（高分辨率） ==========
    print("  [1/6] 生成主布局图...")
    fig1, ax1 = plt.subplots(figsize=(14, 10), dpi=150)
    
    # 厂区背景
    ax1.add_patch(Rectangle((0, 0), bw, bh, lw=2.0, ec='#2C3E50', 
                             fc='#FAFCFA', zorder=0))
    ...
    for f in facilities:
        ...
        # 阴影
        shadow = Rectangle((f.x + 0.6, f.y - 0.6), w, h,
                           fc='#00000015', ec='none', zorder=1.9)
        ...
        rect = FancyBboxPatch(
            ...
            lw=1.0, ec='#333333', fc=color, alpha=0.85, zorder=2
        )
        ax1.add_patch(rect)
        ...
        cx, cy = f.x + w/2, f.y + h/2
        fs = max(7, min(10, int(np.sqrt(f.area) / 2.0)))
        ax1.text(cx, cy, f.name, ha='center', va='center',
                fontsize=fs, fontweight='bold', color='#000000', zorder=10,
                family='monospace')
    ...
    for cat in CATEGORY_NAMES:
        if cat in cat_areas:
            legend_elements.append(mpatches.Patch(
                ...
                label=f"{zone_names_en.get(CATEGORY_NAMES[cat], CATEGORY_NAMES[cat])}"
            ))
    
    ax1.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(1.02, 1.0),
              fontsize=9, framealpha=0.95, edgecolor='#666666',
              title='Functional Zones', title_fontsize=10)
    
    ax1.set_xlim(-5, bw + 5)
    ax1.set_ylim(-5, bh + 5)
    ax1.set_xlabel('X Coordinate (m)', fontsize=11, family='serif')
    ax1.set_ylabel('Y Coordinate (m)', fontsize=11, family='serif')
    ax1.set_title(f'Fig. 1  Optimized Facility Layout Plan\n'
                  f'({bw:.0f}m × {bh:.0f}m, Utilization: {utilization:.1f}%)',
                 fontsize=13, fontweight='bold', family='serif', pad=15)
    ax1.set_aspect('equal')
    ax1.grid(True, ls=':', alpha=0.3, lw=0.5)
    
    plt.tight_layout()
    plt.savefig(f'{save_prefix}_fig1_layout.png', dpi=300, bbox_inches='tight', 
                facecolor='white', pad_inches=0.2)
    plt.close()
    
    # ========== 图2: 收敛曲线分析 ==========
    print("  [2/6] 生成收敛曲线分析...")
    fig2, (ax2a, ax2b) = plt.subplots(1, 2, figsize=(12, 5), dpi=150)
    
    # 左：目标函数曲线（惩罚值，越低越好）
    generations = range(len(history))
    # 将惩罚值转换为正向指标：取负值使其变为越高越好
    # 或者直接展示目标函数值（越低越好）
    ax2a.plot(generations, history, color='#2C3E50', lw=1.8, label='Objective Value')
    ax2a.fill_between(generations, history, alpha=0.15, color='#3498DB')
    
    # 标记关键点（最小值为最优）
    best_gen = np.argmin(history)
    ax2a.scatter([best_gen], [history[best_gen]], color='#27AE60', s=80, zorder=5,
                marker='*', label=f'Best: Gen {best_gen}')
    ax2a.axhline(y=history[-1], color='#E74C3C', ls='--', lw=1, alpha=0.7,
                label=f'Final: {history[-1]:.0f}')
    
    ax2a.set_xlabel('Generation', fontsize=11, family='serif')
    ax2a.set_ylabel('Objective Function Value (Lower is Better)', fontsize=11, family='serif')
    ax2a.set_title('(a) Optimization Convergence Curve', fontsize=12, fontweight='bold', 
                  family='serif', pad=10)
    ...
    # 右：改进量曲线（目标函数减少量，正值表示改进）
    improvements = [0] + [max(0, history[i-1] - history[i]) for i in range(1, len(history))]
    ax2b.bar(generations, improvements, color='#27AE60', alpha=0.7, width=1.0)
    ax2b.set_xlabel('Generation', fontsize=11, family='serif')
    ax2b.set_ylabel('Objective Reduction (Improvement)', fontsize=11, family='serif')
    ax2b.set_title('(b) Generation-wise Improvement', fontsize=12, fontweight='bold',
                  family='serif', pad=10)
    ax2b.grid(True, ls=':', alpha=0.4, axis='y')
    ax2b.spines['top'].set_visible(False)
    ax2b.spines['right'].set_visible(False)
    
    plt.suptitle('Fig. 2  Optimization Convergence Analysis', fontsize=13, 
                fontweight='bold', y=1.02, family='serif')
    plt.tight_layout()
    plt.savefig(f'{save_prefix}_fig2_convergence.png', dpi=300, bbox_inches='tight',
                facecolor='white', pad_inches=0.2)
    plt.close()
    
    # ========== 图3: 分区面积分析 ==========
    print("  [3/6] 生成分区面积分析...")
    fig3, (ax3a, ax3b) = plt.subplots(1, 2, figsize=(13, 5.5), dpi=150)
    
    # 左：饼图
    categories = list(cat_areas.keys())
    areas = [cat_areas[c] for c in categories]
    colors = [CATEGORY_COLORS.get(c, '#CCCCCC') for c in categories]
    labels = [zone_names_en.get(CATEGORY_NAMES[c], CATEGORY_NAMES[c]) for c in categories]
    
    wedges, texts, autotexts = ax3a.pie(areas, labels=None, autopct='%1.1f%%',
                                        colors=colors, startangle=90,
                                        wedgeprops=dict(edgecolor='white', lw=1.5),
                                        pctdistance=0.75, textprops={'fontsize': 9})
    ax3a.legend(wedges, labels, loc='center left', bbox_to_anchor=(1.0, 0.5),
               fontsize=8, framealpha=0.9)
    ax3a.set_title('(a) Area Distribution by Zone', fontsize=12, fontweight='bold',
                  family='serif', pad=10)
    
    # 右：水平柱状图
    y_pos = range(len(categories))
    bars = ax3b.barh(y_pos, areas, color=colors, edgecolor='#333333', lw=0.5, height=0.7)
    ax3b.set_yticks(y_pos)
    ax3b.set_yticklabels(labels, fontsize=9, family='serif')
    ax3b.set_xlabel('Area (m²)', fontsize=11, family='serif')
    ax3b.set_title('(b) Zone Area Comparison', fontsize=12, fontweight='bold',
                  family='serif', pad=10)
    ...
    for bar, area in zip(bars, areas):
        ax3b.text(bar.get_width() + 15, bar.get_y() + bar.get_height()/2,
                 f'{area:.0f}m²', va='center', fontsize=9, family='serif')
    
    plt.suptitle('Fig. 3  Functional Zone Area Analysis', fontsize=13,
                fontweight='bold', y=1.02, family='serif')
    plt.tight_layout()
    plt.savefig(f'{save_prefix}_fig3_zones.png', dpi=300, bbox_inches='tight',
                facecolor='white', pad_inches=0.2)
    plt.close()
    
    # ========== 图4: 相邻性分析 ==========
    print("  [4/6] 生成工艺相邻性分析...")
    fig4, (ax4a, ax4b) = plt.subplots(1, 2, figsize=(13, 5.5), dpi=150)
    
    # 左：相邻距离分布
    distances = [d[2] for d in adj_distances]
    satisfied = [d for d in adj_distances if d[2] < 5]
    not_satisfied = [d for d in adj_distances if d[2] >= 5]
    
    bins = [0, 5, 10, 20, 50, 100, max(distances)+10]
    hist_data, bin_edges = np.histogram(distances, bins=bins)
    
    bar_colors = ['#27AE60', '#F39C12', '#E74C3C', '#E74C3C', '#E74C3C', '#E74C3C']
    ax4a.bar(range(len(hist_data)), hist_data, color=bar_colors[:len(hist_data)],
            edgecolor='#333333', lw=0.5)
    ax4a.set_xticks(range(len(hist_data)))
    ax4a.set_xticklabels(['0-5m\n(Adjacent)', '5-10m', '10-20m', '20-50m', '50-100m', '>100m'],
                        fontsize=9)
    ax4a.set_xlabel('Distance Range', fontsize=11, family='serif')
    ax4a.set_ylabel('Number of Pairs', fontsize=11, family='serif')
    ax4a.set_title('(a) Process Adjacency Distance Distribution', fontsize=12,
    ...
    for i, v in enumerate(hist_data):
        if v > 0:
            ax4a.text(i, v + 0.2, str(v), ha='center', fontsize=10, fontweight='bold')
    
    # 右：相邻性满足率（环形图）
    satisfied_count = len(satisfied)
    not_satisfied_count = len(not_satisfied)
    total_pairs = len(adj_distances)
    
    sizes = [satisfied_count, not_satisfied_count]
    colors_pie = ['#27AE60', '#E74C3C']
    labels_pie = [f'Satisfied ({satisfied_count})', f'Not Satisfied ({not_satisfied_count})']
    
    wedges, texts = ax4b.pie(sizes, labels=None, colors=colors_pie, startangle=90,
                             wedgeprops=dict(width=0.4, edgecolor='white', lw=2))
    
    # 中心文字
    ax4b.text(0, 0, f'{adj_rate:.1f}%\nSatisfied', ha='center', va='center',
             fontsize=14, fontweight='bold', family='serif')
    ax4b.legend(wedges, labels_pie, loc='center left', bbox_to_anchor=(1.0, 0.5),
               fontsize=10)
    ax4b.set_title('(b) Process Adjacency Satisfaction Rate', fontsize=12,
                  fontweight='bold', family='serif', pad=10)
    
    plt.suptitle('Fig. 4  Process Flow Adjacency Analysis', fontsize=13,
                fontweight='bold', y=1.02, family='serif')
    plt.tight_layout()
    plt.savefig(f'{save_prefix}_fig4_adjacency.png', dpi=300, bbox_inches='tight',
                facecolor='white', pad_inches=0.2)
    plt.close()
    
    # ========== 图5: 设施尺寸与空间分析 ==========
    print("  [5/6] 生成设施尺寸分析...")
    fig5, (ax5a, ax5b) = plt.subplots(1, 2, figsize=(13, 5.5), dpi=150)
    
    # 左：设施面积分布（前15个最大设施）
    sorted_facilities = sorted(facilities, key=lambda x: x.area, reverse=True)[:15]
    f_names = [f.name for f in sorted_facilities]
    f_areas = [f.area for f in sorted_facilities]
    f_colors = [CATEGORY_COLORS.get(f.category, '#CCCCCC') for f in sorted_facilities]
    
    bars = ax5a.barh(range(len(f_names)), f_areas, color=f_colors, 
                    edgecolor='#333333', lw=0.5, height=0.7)
    ax5a.set_yticks(range(len(f_names)))
    ax5a.set_yticklabels(f_names, fontsize=9, family='monospace')
    ax5a.set_xlabel('Facility Area (m²)', fontsize=11, family='serif')
    ax5a.set_title('(a) Top 15 Facilities by Area', fontsize=12, fontweight='bold',
                  family='serif', pad=10)
    ax5a.spines['top'].set_visible(False)
    ax5a.spines['right'].set_visible(False)
    ax5a.invert_yaxis()
    
    for bar, area in zip(bars, f_areas):
        ax5a.text(bar.get_width() + 10, bar.get_y() + bar.get_height()/2,
                 f'{area:.0f}m²', va='center', fontsize=8, family='serif')
    ...
    ax5b.hist(aspect_ratios, bins=10, color='#3498DB', edgecolor='#2C3E50',
             alpha=0.8, lw=1)
    ax5b.axvline(x=np.mean(aspect_ratios), color='#E74C3C', ls='--', lw=2,
                label=f'Mean: {np.mean(aspect_ratios):.2f}')
    ax5b.set_xlabel('Aspect Ratio (Length/Width)', fontsize=11, family='serif')
    ax5b.set_ylabel('Number of Facilities', fontsize=11, family='serif')
    ax5b.set_title('(b) Facility Aspect Ratio Distribution', fontsize=12,
                  fontweight='bold', family='serif', pad=10)
    ax5b.legend(loc='upper right', fontsize=9)
    ax5b.spines['top'].set_visible(False)
    ax5b.spines['right'].set_visible(False)
    
    plt.suptitle('Fig. 5  Facility Dimension Analysis', fontsize=13,
                fontweight='bold', y=1.02, family='serif')
    plt.tight_layout()
    plt.savefig(f'{save_prefix}_fig5_dimensions.png', dpi=300, bbox_inches='tight',
                facecolor='white', pad_inches=0.2)
    plt.close()
    
    # ========== 图6: 综合指标仪表盘 ==========
    print("  [6/6] 生成综合指标仪表盘...")
    fig6, axes = plt.subplots(2, 3, figsize=(14, 9), dpi=150)
    
    # 指标数据
    metrics = [
        ('Space Utilization', utilization, '%', 100, '#27AE60'),
        ('Adjacency Rate', adj_rate, '%', 100, '#3498DB'),
        ('Total Facilities', len(facilities), '', 40, '#9B59B6'),
        ('Layout Width', bw, 'm', 250, '#E67E22'),
        ('Layout Height', bh, 'm', 150, '#1ABC9C'),
        ('Total Area', layout_area, 'm²', 20000, '#E74C3C')
    ]
    
    for ax, (name, value, unit, max_val, color) in zip(axes.flat, metrics):
        # 绘制进度环
        theta = np.linspace(0, 2*np.pi, 100)
        r_outer, r_inner = 1.0, 0.6
        
        # 背景环
        ax.fill_between(theta, r_inner, r_outer, alpha=0.15, color='#CCCCCC')
        
        # 进度环
        progress = min(value / max_val, 1.0)
        theta_progress = np.linspace(np.pi/2, np.pi/2 - 2*np.pi*progress, 100)
        for i in range(len(theta_progress)-1):
            ax.fill_between([theta_progress[i], theta_progress[i+1]], 
                           [r_inner, r_inner], [r_outer, r_outer], 
                           color=color, alpha=0.8)
        
        # 简化：用饼图模拟
        ax.clear()
        sizes = [progress, 1-progress]
        colors_gauge = [color, '#EEEEEE']
        wedges, _ = ax.pie(sizes, colors=colors_gauge, startangle=90,
                          wedgeprops=dict(width=0.35, edgecolor='white', lw=2))
        
        # 中心文字
        if unit == '%':
            ax.text(0, 0.1, f'{value:.1f}{unit}', ha='center', va='center',
                   fontsize=18, fontweight='bold', color=color, family='serif')
        elif unit == 'm²':
            ax.text(0, 0.1, f'{value:.0f}', ha='center', va='center',
                   fontsize=18, fontweight='bold', color=color, family='serif')
            ax.text(0, -0.2, unit, ha='center', va='center',
                   fontsize=12, color='#666666', family='serif')
        else:
            ax.text(0, 0.1, f'{value:.0f}{unit}', ha='center', va='center',
                   fontsize=18, fontweight='bold', color=color, family='serif')
        
        ax.text(0, -0.45, name, ha='center', va='center',
               fontsize=11, fontweight='bold', color='#333333', family='serif')
        ax.set_xlim(-1.3, 1.3)
        ax.set_ylim(-1.3, 1.3)
        ax.set_aspect('equal')
        ax.axis('off')
    
    plt.suptitle('Fig. 6  Optimization Performance Dashboard', fontsize=14,
                fontweight='bold', y=0.98, family='serif')
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig(f'{save_prefix}_fig6_dashboard.png', dpi=300, bbox_inches='tight',
                facecolor='white', pad_inches=0.2)
    plt.close()
    
    # ========== 输出汇总 ==========
    print("\n" + "-" * 78)
    print("  ✅ 学术论文可视化分析完成！生成文件：")
    print(f"     • {save_prefix}_fig1_layout.png      - 主布局图")
    print(f"     • {save_prefix}_fig2_convergence.png - 收敛曲线分析")
    print(f"     • {save_prefix}_fig3_zones.png       - 分区面积分析")
    print(f"     • {save_prefix}_fig4_adjacency.png   - 工艺相邻性分析")
    print(f"     • {save_prefix}_fig5_dimensions.png  - 设施尺寸分析")
    print(f"     • {save_prefix}_fig6_dashboard.png   - 综合指标仪表盘")
    print("-" * 78)
    
    # 输出关键指标表格（适合论文使用）
    print("\n📋 关键性能指标汇总表 (Table for Academic Paper):")
    print("=" * 60)
    print(f"{'指标 (Metric)':<30}{'数值 (Value)':<15}{'单位 (Unit)'}")
    print("-" * 60)
    print(f"{'Layout Dimensions':<30}{f'{bw:.0f} × {bh:.0f}':<15}{'m'}")
    print(f"{'Total Layout Area':<30}{layout_area:<15.0f}{'m²'}")
    print(f"{'Total Facility Area':<30}{total_facility_area:<15.0f}{'m²'}")
    print(f"{'Space Utilization Rate':<30}{utilization:<15.2f}{'%'}")
    print(f"{'Number of Facilities':<30}{len(facilities):<15}{'units'}")
    print(f"{'Number of Functional Zones':<30}{len(cat_areas):<15}{'zones'}")
    print(f"{'Process Adjacency Pairs':<30}{len(adj_distances):<15}{'pairs'}")
    print(f"{'Adjacency Satisfaction Rate':<30}{adj_rate:<15.1f}{'%'}")
    print(f"{'Optimization Generations':<30}{len(history):<15}{'generations'}")
    print(f"{'Final Fitness Value':<30}{history[-1]:<15.4f}{''}")
    print("=" * 60)
...
def main():
    ...
    ga = AdvancedGA(
        facilities, 
        population_size=200,  # 深度优化
        generations=150,      # 深度优化：150代
        mutation_rate=0.35    # 平衡探索与利用
    )
